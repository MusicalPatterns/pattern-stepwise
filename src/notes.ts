...
const buildStepwiseNoteSpecs: () => NoteSpecsDictionary =
    (): NoteSpecsDictionary => {
        ...
        const threePerNoteSpecs: NoteSpec[] = sequence([ threePer, ninePer ])
            .map(buildStepwiseNoteSpec)
        const fivePerNoteSpecs: NoteSpec[] = fivePer
            .map(buildStepwiseNoteSpec)
        const sevenPerNoteSpecs: NoteSpec[] = sevenPer
            .map(buildStepwiseNoteSpec)
        const ninePerNoteSpecs: NoteSpec[] = sequence([ ninePer, threePer ])
            .map(buildStepwiseNoteSpec)

        const backboneNoteSpecs: NoteSpec[] = backbone
            .map(buildStepwiseNoteSpec)

        const kickNoteSpecs: NoteSpec[] = kick
            .map(buildUnpitchedStepwiseNoteSpec)
        const snareNoteSpecs: NoteSpec[] = snare
            .map(buildUnpitchedStepwiseNoteSpec)
        const hihatNoteSpecs: NoteSpec[] = hihat
            .map(buildUnpitchedStepwiseNoteSpec)
